...
(function () {
  ...
  function createButton(classNames, svgContent, onclickHandler) {
    ...
    classNames.forEach(cls => btn.classList.add(cls));
  ...
  document.querySelectorAll(".header__right .header__link").forEach(el => el.remove());
  ...
  navItems.forEach((item, i) => item.innerHTML = icons[i] || "");
